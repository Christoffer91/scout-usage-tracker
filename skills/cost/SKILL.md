...
The command uses Scout's active `SESSION_ID` automatically. It reports usage before the current request, so the usage of the `/cost` response itself appears next time.
