...
class ConfigRenderTests(unittest.TestCase):
    ...
    def test_redesigned_dashboard_is_self_contained_and_accessible(self):
        ...
        self.assertIn(".filter-summary.is-concealed { visibility: hidden; }", text)
        self.assertIn(".show-all-models.is-concealed { visibility: hidden; pointer-events: none; }", text)
        self.assertIn('data-filter-summary aria-hidden="true">Filtered view</span>', text)
        self.assertIn('data-show-all aria-hidden="true" tabindex="-1">Show all</button>', text)
        self.assertNotIn("summary.hidden", text)
        self.assertNotIn("showAll.hidden", text)
        self.assertIn("summary.classList.remove('is-concealed')", text)
        self.assertIn("Last ${periodDays} days", text)
        self.assertIn("showAll.classList.toggle('is-concealed', allModelsIncluded)", text)
        self.assertIn(".hero-kpi small { display: block; min-height: 3em;", text)
        self.assertIn("const reserveDashboardHeight = () =>", text)
        self.assertIn("stableDashboardHeight = Math.max(stableDashboardHeight, dashboard.scrollHeight)", text)
        self.assertIn("currentPeriodRecords = recordsForPeriod(periodDays);", text)
        self.assertIn("const view = combined(currentPeriodRecords);", text)
        ...
        self.assertEqual(text.count('class="bar-hit"'), 30)
        self.assertEqual(text.count('bar-fill empty'), 29)
        self.assertNotIn("Unusually high day", text.split("<script>", 1)[0])
        self.assertEqual(text.count('data-period-days="'), 6)
        self.assertIn('data-period-days="30" aria-pressed="true">30d</button>', text)
        self.assertIn("const allowedPeriods = [7, 14, 30, 60, 90, 365]", text)
        self.assertIn("localStorage.getItem('scout-period-days')", text)
        ...
        self.assertIn("Week ${row.label}", text)
        self.assertNotIn("ISO week ${row.label}", text)
        self.assertIn(">By week</button>", text)
        self.assertIn("selectedMetric === 'cost' ? 'Estimated Scout cost'", text)
        self.assertNotIn("Estimated Scout cost (${selectedCurrency})", text)
