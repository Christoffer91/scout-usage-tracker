...
class ConfigRenderTests(unittest.TestCase):
    ...
    def test_model_filter_payload_contains_only_aggregate_usage(self):
        ...
        self.assertEqual(set(payload), {"models", "records", "window_end", "money", "chat_names"})
        ...
        self.assertEqual(set(payload["chat_names"]), {record["chat"] for record in payload["records"]})
        self.assertTrue(all(name.startswith("Chat ") for name in payload["chat_names"].values()))
