...
The dashboard link is generated at runtime from the validated local `dashboard_path` using Python's platform-aware `Path.as_uri()`. It is emitted as an escaped HTML hyperlink, so installations on macOS, Linux, and Windows point to their own configured local dashboard instead of a hardcoded path. The ordinary report ends with an invitation to run `/cost FAQ`; detailed rounding, verification, and billing caveats live in that FAQ and in the dashboard rather than crowding every response.
