...
The default report is in English and shows only the current chat summary: model calls, rounded Scout credits, input/output/cache-read tokens, estimated gross USD/NOK value, and a link to the local dashboard. The report is measured **before the `/cost` request**, so the usage of the `/cost` answer itself appears next time. One answer may contain several model calls.
...
/cost FAQ                      # usage guide and accounting caveats only
```
...
The dashboard link is generated at runtime from the validated local `dashboard_path` using Python's platform-aware `Path.as_uri()`. It is emitted as an escaped HTML hyperlink with the short visible label **Usage tracker**, so installations on macOS, Linux, and Windows point to their own configured local dashboard without displaying a long machine-specific URL. The ordinary report ends with an invitation to run `/cost FAQ`; detailed rounding, verification, and billing caveats live in that FAQ and in the dashboard rather than crowding every response.
